...
Apply runs on both editions - Home included, once a mounted CD/DVD (which
had been silently blocking it under a misleading "this edition doesn't
support this feature" error) was ejected. On Home specifically, there is
one thing this toolkit cannot do: initialize BitLocker metadata on a
completely blank volume (e.g. immediately after a full decrypt) - every
scriptable API tried, including the officially-documented MDM `BitLocker`
CSP, fails identically. That first-time initialization needs one manual,
one-time step (flipping Settings > Privacy & security > Device encryption
to On - not completing its Microsoft-account sign-in prompt, just the
toggle); this toolkit reliably takes over from there forever after,
confirmed across multiple Apply runs, reboots, and a full
restore-to-fresh-and-reapply cycle. See the Findings entries for the full
investigation. Run through
